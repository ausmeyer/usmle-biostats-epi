...
pos <- ggplot(mtcars, aes(x=wt, y=hp)) + 
  ...
  ggtitle("Positive Correlation") + 
  labs(x="Weight", y="Horse Power") +
  annotate("text", x = 1.5, y = 300, label = paste("r = ", round(cor(mtcars$wt, mtcars$hp), digits = 3), sep = ''), size=5, hjust = 0) +
  annotate("text", x = 1.5, y = 280, label = paste("r^2 = ", round(cor(mtcars$wt, mtcars$hp)^2, digits = 3), sep = ''), size=5, hjust = 0)
show(pos)
```
